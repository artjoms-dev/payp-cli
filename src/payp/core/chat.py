...
class ChatSession:
    ...
    async def _execute_sql_with_mode(
        ...
        # SELECTs always bypass review/approval
        if classification.category == SqlCategory.SELECT:
        ...
        # For SECURE / SECURE-AUTO: run reviewer first
        review_reason = ""
        approved_sql = sql
        reviewer_override = False
        if self.mode in (SecurityMode.SECURE, SecurityMode.SECURE_AUTO):
            ...
            self.console.print(
                f"  [dim]Reviewer ({self.llm.get_reviewer_model()}): "
                f"[bold]{review.verdict.value.upper()}[/bold] — {review.reason}[/dim]"
            )
            ...
            # SECURE-AUTO with APPROVE → execute without asking user
            if self.mode == SecurityMode.SECURE_AUTO and review.verdict == Verdict.APPROVE:
                ...
                self._log_sql_execution(approved_sql, classification.statement_type, result, approved_by="reviewer")
                return result.data if result.success else {"error": result.error}

            # SECURE-AUTO with SAFER → execute the safer version
            if self.mode == SecurityMode.SECURE_AUTO and review.verdict == Verdict.SAFER:
                display_tool_call(self.console, "execute_sql", {"sql": approved_sql})
                args = {**args, "sql": approved_sql}
                result = await tool.call(args, context)
                display_tool_result(self.console, "execute_sql", result.success, result.summary)
                self._display_tool_data("execute_sql", result, sql=approved_sql)
                self._log_sql_execution(approved_sql, classification.statement_type, result, approved_by="reviewer-safer")
                return result.data if result.success else {"error": result.error}

        # MANUAL mode OR SECURE with non-approve → show approval UI
        warning = None
        ...
        self._log_sql_execution(approval.sql, classification.statement_type, result, approved_by=approved_by)
        return result.data if result.success else {"error": result.error}
